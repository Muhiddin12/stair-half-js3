...
let favoriteUrl = 'https://api.themoviedb.org/3/movie/now_playing?api_key=ffd716e442cea750f989f65854a0cb89&language=en-US&page=1'
fetch(favoriteUrl)
    ...
    .then((another) => {
        ...
        favoriteArray.map((item) => {
            ...
            let favouriteBody = document.createElement('p')
            favouriteBody.textContent = `Rating : ${item.vote_average}`
            favouriteBody.id = 'favouriteBody'
            favoriteItemBody.appendChild(h2Body)
            favoriteItemBody.appendChild(pBody)
            favoriteItemBody.appendChild(favouriteBody)
